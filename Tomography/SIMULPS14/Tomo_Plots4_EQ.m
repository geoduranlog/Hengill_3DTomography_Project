...
%% Load EQ Files -- Used for the tomography (It should match with the loaded Vmodel bellow)
load EQ_S_Tomo %EQ_VpVs_Tomo  %EQ_P_Tomo;  EQ_S_Tomo
...
%Layer2= 1km to 3km
idx=find(EQ(:,3)>=1 & EQ(:,3)<=3);
EQ_Ly2=EQ(idx,:);

%Layer3= 3km to 5km
idx=find(EQ(:,3)>=3 & EQ(:,3)<=5);
EQ_Ly3=EQ(idx,:);
...
%--Size --
size_symb=6e4 %4e4 for S-waves %2e5; %4.  %Size of the symbols used to plot rcv correction.  If 'geom' mode  %ADN
...
abs_max=15  %5;  % 15 for Vp and Vs Tomo.  5 for Vp/Vs or use min maxvalues
...
%--- Earthquakes at this layer -----
e=geoscatter(EQ_Ly1(:,1),EQ_Ly1(:,2),EQsz,'ko') 
e.MarkerFaceColor = rgb('Olive'); % SaddleBrownTomato Salmon Brown  RosyBrown   rgb('Azure'); 
e.MarkerEdgeColor = rgb('Black'); %LightSlateGray
...
%--- Geothermal Plants----
s=geoscatter(lat_geoplant,lon_geoplant,A_gplant,'d','filled')
...
%--- Earthquakes at this layer -----
e=geoscatter(EQ_Ly2(:,1),EQ_Ly2(:,2),EQsz,'ko') 
e.MarkerFaceColor = rgb('Olive'); % SaddleBrownTomato Salmon Brown  RosyBrown   rgb('Azure'); 
e.MarkerEdgeColor = rgb('Black'); %LightSlateGray
...
%--- Earthquakes at this layer -----
e=geoscatter(EQ_Ly3(:,1),EQ_Ly3(:,2),EQsz,'ko') 
e.MarkerFaceColor = rgb('Olive'); % SaddleBrownTomato Salmon Brown  RosyBrown   rgb('Azure'); 
e.MarkerEdgeColor = rgb('Black'); %LightSlateGray
...
%--- Earthquakes at this layer -----
e=geoscatter(EQ_Ly4(:,1),EQ_Ly4(:,2),EQsz,'ko') 
e.MarkerFaceColor = rgb('Olive'); % SaddleBrownTomato Salmon Brown  RosyBrown   rgb('Azure'); 
e.MarkerEdgeColor = rgb('Black'); %LightSlateGray
...
save_folder='/Users/alejandro/ICELAND_Project/Tomography/Figures/Paper_Proposal_Fig/FIgs_Corrections2021/work/';
%print(gcf,[save_folder,'Tomo_Vs_EQ_StacorrTst'],'-dpdf','-r800');  %dpi set at 800 to increase image resolution 
