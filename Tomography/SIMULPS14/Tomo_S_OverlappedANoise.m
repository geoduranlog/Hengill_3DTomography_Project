%==== Plot Contourn of Amb Noise Tomography over Body waves Tomography====
...
%% ======= LOAD Ambient Noise Tomo (Pilar) at z=2km depth ====

load '/Users/alejandro/ICELAND_Project/Tomography/Pilar_files_tomo_CheckHDISK_MyPassport/Pili_Vs_ANoise_2km.mat';

%Test
%{
contourm(Vs_2km.lat,Vs_2km.lon,Vs_2km.v,'Linewidth',2.5)
colorbar

contourm(Vs_2km.lat,Vs_2km.lon,Vs_2km.v,'color',rgb('Gray'),'Linewidth',2.5)
%}
...
abs_max=15  %5;  % 15 for Vp and Vs Tomo.  5 for Vp/Vs
...
%----Plane 2----
Vs_2kref=3.4; %km/s ->Body S-tomo Ref value at 2km
  subplot(2,2,2)
colormap(customCMap);
%colormap(colormap_TomoP);
...
s.MarkerFaceColor = rgb('Lime');
s.MarkerEdgeColor = rgb('Black');
%----Amb Noise Contourn z=2km----
%contourm(Vs_2km.lat,Vs_2km.lon,Vs_2km.v,'Linewidth',2.5)
% for i=1:52
%     j=1:52
% geoplot(Vs_2km.lat(i),Vs_2km.lon(i),Vs_2km.v(i)) %,'color',rgb('Gray'),'Linewidth',2.5)
% %geoscatter(Vs_2km.lat(i),Vs_2km.lon(i),Vs_2km.v(i)) 
% end
...
geolimits(latz,lonz);
caxis([-abs_max  abs_max])
%caxis([Vs_2kref*(1-abs_max/100)  Vs_2kref*(1+abs_max/100)])  %Nominal Vs value
title('z=2.0km')
%dp.Parent.LatitudeAxis.Visible='off'
%dp.Parent.LongitudeAxis.Visible='off'
ylabel(colb,'dv_{P} (%)')
set(get(colb,'label'),'string','V_{S}(km/s)','Rotation',90);
...
[c,h] =contourm(Vs_2km.lat,Vs_2km.lon,Vs_2km.v,'ShowText','on','Linewidth',2.5,'Linecolor','k');
...
ylim([latz])
%caxis([-abs_max  abs_max])
%caxis([min(min(Vs_2km.v))  max(max(Vs_2km.v))])
caxis([Vs_2kref*(1-abs_max/100)  Vs_2kref*(1+abs_max/100)]) 
%caxis([ 2.3855  3.91])%To match the Tomo scale 
title('z=4.0km')
grid off
...
save_folder='/Users/alejandro/ICELAND_Project/Tomography/Figures/';
%print(gcf,[save_folder,'Tomo_contourn_black'],'-dpdf','-r800');  %dpi set at 800 to increase image resolution 
